...
function initDrawingSystem() {
    ...
    function resizeDrawingCanvas() {
        ...
        const prevData = drawingCtx.getImageData(0, 0, drawingCanvas.width || 1, drawingCanvas.height || 1);
...
function handleDrawingStart(e) {
    ...
    if (pigeonGameActive) return; // Não desenha se o minigame estiver ativo
...
const SoundFX = {
    ...
    startRetroMusic() {
        ...
        musicInterval = setInterval(() => {
            ...
            if (step % 4 === 0) {
                SoundFX.playNote(130.8, 'square', 0.08, 0.05); // Bass beat
            }
            step++;
        }, 180);
    },
...
function initPigeonEasterEgg() {
    ...
    // Adiciona evento de clique e touch
...
function createPigeonHUD() {
    ...
    pigeonHud.innerHTML = `
        ...
        <div class="hud-controls">
            ...
            <button id="pigeon-exit-btn" class="hud-btn hud-btn-exit" title="Sair do Jogo">✕ Fechar</button>
        </div>
    `;
...
function startPigeonGame() {
    ...
    // Mostra aviso inicial rápido
    showBannerMessage('🎯 PEGUE O POMBO! Clique nele para abater!');
    ...
    // Efeito de tiro ao clicar em qualquer lugar durante o jogo
    window.addEventListener('click', handleGameShot);
}

function handleGameShot(e) {
    if (!pigeonGameActive) return;
    if (e.target && e.target.closest('#pigeon-hud')) return;
    SoundFX.playShoot();
}
...
function createPigeon() {
    ...
    // Eventos de clique e toque no pombo
...
function animatePigeon() {
    ...
    // Bate nas bordas com amortecimento e rotação
    ...
    if (newY <= boundMargin + 60) { // Deixa espaço para o HUD
    ...
    // Curvas aleatórias suaves para voo realista
    if (Math.random() < 0.03) {
        const randAngle = (Math.random() - 0.5) * 1.2;
    ...
    // Efeito de bater de asas e giro
    const wingFlap = Math.sin(Date.now() / 120) * 10;
...
function killPigeon() {
    ...
    // Explosão de penas / partículas
    createFeatherExplosion(centerX, centerY);
    showFloatingPoints(centerX, centerY, `+100 🎯`);
    
    pigeonElement.style.transition = 'transform 0.4s ease-out, opacity 0.4s ease-out';
    ...
    setTimeout(() => {
        ...
        // Aumenta a velocidade do próximo pombo
        pigeonSpeed = Math.min(pigeonSpeed + 0.6, 12);
